//
//  KindergartenEditViewController.swift
...
class KindergartenEditViewController:BaseViewController {
    ...
    private var kindergartenActiveInactiveBGView : UIView = UIView()
    ...
    private var clickedTextView: UITextView?
    ...
    private var activateBtn : UIButton = UIButton()
    private var inactivateBtn : UIButton = UIButton()
    private var submitBtn : UIButton = UIButton()
    ...
    private var isChildNotCompulsaryAgeCheck: Bool = false
    private var isApplicable : Bool = false
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "Informationen bearbeiten"
    ...
    func configSetUpUI(){
        ...
        spouseNotGettingAllowanceBtn.addTarget(self, action: #selector(childNotCompulsaryAgeBtnAction(sender:)), for: .touchUpInside)
        ...
        let kindergartenActiveInactiveBGView: UIView = UIView()
        kindergartenActiveInactiveBGView.layer.cornerRadius = 2 * AutoSizeScaleX
        kindergartenActiveInactiveBGView.layer.shadowColor = UIColor.black.cgColor
        kindergartenActiveInactiveBGView.layer.shadowOffset = CGSize.zero
        kindergartenActiveInactiveBGView.layer.shadowOpacity = 0.12
        kindergartenActiveInactiveBGView.layer.shadowRadius = 3
        self.configView.addSubview(kindergartenActiveInactiveBGView)
        self.kindergartenActiveInactiveBGView = kindergartenActiveInactiveBGView
        kindergartenActiveInactiveBGView.snp.makeConstraints { (make) in
            make.top.equalTo(kindergartenAddressBGView.snp_bottom).offset(20 * AutoSizeScaleX)
            make.height.equalTo(80 * AutoSizeScaleX)
            make.left.equalTo(self.configView).offset(20 * AutoSizeScaleX)
            make.right.equalTo(self.configView).offset(-20 * AutoSizeScaleX)
        }
        
        let activeInactiveNotifyMsgLab: UILabel = UILabel()
        activeInactiveNotifyMsgLab.textColor = .lightGray
        activeInactiveNotifyMsgLab.font = UIFont.appFont(ofSize: 12 * AutoSizeScaleX, weight: .regular)
        activeInactiveNotifyMsgLab.text = " "
        activeInactiveNotifyMsgLab.textAlignment = .left
        activeInactiveNotifyMsgLab.numberOfLines = 0
        kindergartenActiveInactiveBGView.addSubview(activeInactiveNotifyMsgLab)
        activeInactiveNotifyMsgLab.snp.makeConstraints { (make) in
            make.left.right.equalTo(kindergartenActiveInactiveBGView)
            make.height.equalTo(4 * AutoSizeScaleX)
            make.top.equalTo(kindergartenActiveInactiveBGView).offset(4 * AutoSizeScaleX)
        }
        
        let activateBtn: UIButton = UIButton(type: .custom)
        activateBtn.setTitleColor(UIColor.lightGray, for: .normal)
        activateBtn.setTitle( "Aktiv", for: .normal)
        activateBtn.layer.cornerRadius = 7 * AutoSizeScaleX
        activateBtn.layer.borderWidth = 1 * AutoSizeScaleX
        activateBtn.layer.borderColor = UIColor.lightGray.cgColor
        activateBtn.layer.shadowColor = UIColor.black.cgColor
        activateBtn.layer.shadowOffset = CGSize(width: 0, height: 3)
        activateBtn.layer.shadowOpacity = 0.12
        activateBtn.layer.shadowRadius = 3
        activateBtn.addTarget(self, action: #selector(activateBtnAction(sender:)), for: .touchUpInside)
        self.kindergartenActiveInactiveBGView.addSubview(activateBtn)
        self.activateBtn = activateBtn
        activateBtn.snp.makeConstraints { (make) in
            make.left.equalTo(self.configView).offset(20 * AutoSizeScaleX)
            make.top.equalTo(activeInactiveNotifyMsgLab.snp_bottom).offset(10 * AutoSizeScaleX)
            make.height.equalTo(46 * AutoSizeScaleX)
            make.width.equalTo(110 * AutoSizeScaleX)
        }
        
        let inactivateBtn: UIButton = UIButton(type: .custom)
        inactivateBtn.setTitleColor(UIColor.red, for: .normal)
        inactivateBtn.setTitle( "Inaktiv", for: .normal)
        inactivateBtn.layer.cornerRadius = 7 * AutoSizeScaleX
        inactivateBtn.layer.borderWidth = 1 * AutoSizeScaleX
        inactivateBtn.layer.borderColor = UIColor.red.cgColor
        inactivateBtn.layer.shadowColor = UIColor.black.cgColor
        inactivateBtn.layer.shadowOffset = CGSize(width: 0, height: 3)
        inactivateBtn.layer.shadowOpacity = 0.12
        inactivateBtn.layer.shadowRadius = 3
        inactivateBtn.addTarget(self, action: #selector(inactivateBtnAction(sender:)), for: .touchUpInside)
        self.kindergartenActiveInactiveBGView.addSubview(inactivateBtn)
        self.inactivateBtn = inactivateBtn
        inactivateBtn.snp.makeConstraints { (make) in
            make.right.equalTo(self.configView).offset(-20 * AutoSizeScaleX)
            make.top.equalTo(activeInactiveNotifyMsgLab.snp_bottom).offset(6 * AutoSizeScaleX)
            make.height.equalTo(46 * AutoSizeScaleX)
            make.width.equalTo(130 * AutoSizeScaleX)
        }
        
        if(self.getkindergartenKidsDetails?.is_applicable == false){
            activateBtn.layer.borderColor = UIColor.lightGray.cgColor
            activateBtn.setTitleColor(UIColor.lightGray, for: .normal)
            inactivateBtn.setTitleColor(UIColor.red, for: .normal)
            inactivateBtn.layer.borderColor = UIColor.red.cgColor
        }else{
            activateBtn.layer.borderColor = UIColor(hexString: "#3868F6").cgColor
            activateBtn.setTitleColor(UIColor(hexString: "#3868F6"), for: .normal)
            inactivateBtn.setTitleColor(UIColor.lightGray, for: .normal)
            inactivateBtn.layer.borderColor = UIColor.lightGray.cgColor
        }
        ...
        submitBtn.snp.makeConstraints { (make) in
            make.right.equalTo(self.configView).offset(-20 * AutoSizeScaleX)
            make.left.equalTo(self.configView).offset(20 * AutoSizeScaleX)
            make.top.equalTo(kindergartenActiveInactiveBGView.snp_bottom).offset(40 * AutoSizeScaleX)
            make.height.equalTo(46 * AutoSizeScaleX)
        }
    ...
    func updateUI(){
        ...
        self.spouseNotGettingAllowanceBtn.isSelected = self.getkindergartenKidsDetails!.spouse_not_getting_benefit
        self.childNotCompulsaryAgeBtn.isSelected = self.getkindergartenKidsDetails!.not_compulsory_school_age
        self.isChildNotCompulsaryAgeCheck = self.getkindergartenKidsDetails!.not_compulsory_school_age
        self.isApplicable = self.getkindergartenKidsDetails!.is_applicable
        self.nameTF.text = self.getkindergartenKidsDetails?.name
        self.selectedGenderLab.text = self.getkindergartenKidsDetails?.gender
        self.selectedDobLab.text = self.getkindergartenKidsDetails?.dob
        self.datePicker.date = self.getDateFromString(date: self.selectedDobLab.text!)
        self.kindergartenNameTypeTF.text = self.getkindergartenKidsDetails?.kinder_garden_name
        self.kindergartenAddressTextView.text = self.getkindergartenKidsDetails?.kinder_garden_address
        self.placeholderLabel.isHidden = true
        if(self.getkindergartenKidsDetails?.gender == "male"||self.getkindergartenKidsDetails?.gender == "Männlich"){
...
//      }
    ...
    @objc func datePickerValueChanged() {
        if(self.isChildNotCompulsaryAgeCheck == false){
            self.datePicker.minimumDate = Calendar.current.date(byAdding: .year, value: -10, to: Date())
        }else{
            self.datePicker.minimumDate = Calendar.current.date(byAdding: .year, value: -7, to: Date())
        }
 ...
 //       kindergartenStore.childNotCompulsory    = self.childNotCompulsaryAgeBtn.isSelected == true
        ...
        do {
            ...
            ApplicationDelegate.client.editKidKindergarten(id: self.getkindergartenKidsDetails!.id, user_id: self.getkindergartenKidsDetails!.user_id, name: self.nameTF.text!, gender: self.selectedGenderLab.text!, dob: self.selectedDobLab.text!, age: self.getkindergartenKidsDetails!.age, kinder_garden_name: self.kindergartenNameTypeTF.text!, kinder_garden_address: self.kindergartenAddressTextView.text!, is_applicable: self.isApplicable, not_compulsory_school_age: self.isChildNotCompulsaryAgeCheck, spouse_not_getting_benefit: self.spouseNotGettingAllowanceBtn.isSelected, created_at: "", updated_at: "", deleted_at: ""){ message in
    ...
    @objc func activateBtnAction(sender: UIButton){
    ...
    @objc func inactivateBtnAction(sender: UIButton){
...
extension KindergartenEditViewController: UITextFieldDelegate {
...
extension KindergartenEditViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(self.getkindergartenKidsDetails?.gender == self.genderListArray[indexPath.row]){
...
extension KindergartenEditViewController: UITextViewDelegate {
