...
class ICToast: NSObject {
    ...
    class func showToastAction() {
        if Thread.main.isMainThread {
            ...
            toastView?.frame = CGRect.init(x: (Screen_Width - 70) / 2, y: (Screen_Height - 70) / 2, width: 70, height: 70)
            toastView?.alpha = 1
        } else {
    ...
    class func showToast(message: NSString?, aLocationStr: NSString?, aShowTime: TimeInterval) {
        if self.isEmptyStr(str: message as String?) {
            return
        }
        if Thread.current.isMainThread {
            let appDelegate: AppDelegate? = UIApplication.shared.delegate as? AppDelegate
            if appDelegate?.isNeedRotation ?? false {
                showHorizontalToast(message: message, aLocationStr: aLocationStr, aShowTime: aShowTime)
                return
            }
            ...
            if aLocationStr != nil, aLocationStr == "top" {
                labFrame = CGRect.init(x: (Screen_Width - width) / 2, y: Screen_Height * 0.15, width: width, height: height)
            } else if aLocationStr != nil, aLocationStr == "bottom" {
                labFrame = CGRect.init(x: (Screen_Width - width) / 2, y: Screen_Height * 0.85, width: width, height: height)
            } else {
                //default-->center
                labFrame = CGRect.init(x: (Screen_Width - width) / 2, y: Screen_Height * 0.5, width: width, height: height)
            }
    ...
    //横屏显示消息
    class func showHorizontalToast(message: NSString?, aLocationStr: NSString?, aShowTime: TimeInterval) {
        if Thread.current.isMainThread {
            toastLabel = self.currentToastLabel()
            toastLabel?.removeFromSuperview()
            ...
            var labFrame = CGRect.zero
            if aLocationStr != nil, aLocationStr == "top" {
                labFrame = CGRect.init(x: (Screen_Height - width) / 2, y: Screen_Width * 0.15, width: width, height: height)
            } else if aLocationStr != nil, aLocationStr == "bottom" {
                labFrame = CGRect.init(x: (Screen_Height - width) / 2, y: Screen_Width * 0.85, width: width, height: height)
            } else {
                //default-->center
                labFrame = CGRect.init(x: (Screen_Height - width) / 2, y: Screen_Width * 0.5, width: width, height: height)
            }
            toastLabel?.frame = labFrame
            toastLabel?.text = message as String?
            toastLabel?.alpha = 1
            ...
            DispatchQueue.main.async {
                self.showHorizontalToast(message: message, aLocationStr: aLocationStr, aShowTime: aShowTime)
            }
            return
        }
    }
    ...
    class func showIndicatorToast(message: NSString?, aLocationStr: NSString?, aShowTime: TimeInterval) {
        if Thread.current.isMainThread {
            ...
            var labFrame = CGRect.zero
            if aLocationStr != nil, aLocationStr == "top" {
                labFrame = CGRect.init(x: (Screen_Width - width) / 2, y: Screen_Height * 0.15, width: width, height: 60 + height)
            } else if aLocationStr != nil, aLocationStr == "bottom" {
                labFrame = CGRect.init(x: (Screen_Width - width) / 2, y: Screen_Height * 0.85, width: width, height: 60 + height)
            } else {
                //default-->center
                labFrame = CGRect.init(x: (Screen_Width - width) / 2, y: Screen_Height * 0.5, width: width, height: 60 + height)
            }
...
extension ICToast {
    ...
    class func currentToastLabel() -> UILabel {
        ...
        if toastLabel == nil {
            ...
            toastLabel?.font = UIFont.appFont(ofSize: 16, weight: .regular)
    ...
    class func currentToastViewLabel() -> UIView {
        ...
        if toastViewLabel == nil {
            ...
            aLabel.font = UIFont.appFont(ofSize: 16, weight: .regular)
